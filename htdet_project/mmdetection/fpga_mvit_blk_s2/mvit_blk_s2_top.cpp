/*
 * mvit_blk_s2_top.cpp
 * Full MobileViT Block Stage-2: local conv + proj + 4×(2×transformer) + back-proj + fusion
 * Self-contained: includes transformer_block_s2 and mobilevit_block_s2 directly.
 *
 * ⚠ Synthesis warning: this is a large module.
 *   BRAM estimate: ~600–900 BRAM_18K (40–50% of xczu28dr)
 *   LUT estimate: ~200–300K (50–70%)
 *   Expected synthesis time: 15–30 min
 */
...
// ── transformer_block_s2 ──────────────────────────────────────────────────────
static void transformer_block_s2(act_t* tokens, const meta_t* w) {
    #pragma HLS INLINE
    ...
    const int N = MVIT_N_S2, dim = MVIT_S2_DIM, head_dim = dim / MVIT_HEADS;
    int off = 0;
    ...
    S2T_MHA: for (int h = 0; h < MVIT_HEADS; h++) {
        ...
        S2T_AQ: for (int q = 0; q < N; q++) {
            S2T_AK: for (int k = 0; k < N; k++) {
                ...
                acc_t dot = 0;
                S2T_AD: for (int d = 0; d < head_dim; d++) { #pragma HLS UNROLL factor=4
            ...
            S2T_VD: for (int d = 0; d < head_dim; d++) {
                #pragma HLS PIPELINE II=1
                acc_t sum = 0;
                S2T_VK: for (int k = 0; k < N; k++) { #pragma HLS UNROLL factor=4
    ...
    S2T_R1: for (int i = 0; i < N*dim; i++) { #pragma HLS PIPELINE II=1 tokens[i] += s2t_proj_out[i]; }
    ...
    S2T_M1B: for (int b = 0; b < N; b++) {
        S2T_M1O: for (int od = 0; od < hidden; od++) {
            ...
            S2T_M1I: for (int id = 0; id < dim; id++) { #pragma HLS UNROLL factor=8
    ...
    S2T_M2B: for (int b = 0; b < N; b++) {
        S2T_M2O: for (int od = 0; od < dim; od++) {
            ...
            S2T_M2I: for (int id = 0; id < hidden; id++) { #pragma HLS UNROLL factor=8
                acc += (acc_t)s2t_mlp_hid[b*hidden+id] * (acc_t)W2[od*hidden+id]; }
            s2t_proj_out[b*dim+od] = (act_t)acc;
        }
    }
    S2T_R2: for (int i = 0; i < N*dim; i++) { #pragma HLS PIPELINE II=1 tokens[i] += s2t_proj_out[i]; }
}

// ── mobilevit_block_s2 ────────────────────────────────────────────────────────
static void mobilevit_block_s2(
...
void mvit_blk_s2_top(
    ...
    int coff = 0, moff = 0;
    mobilevit_block_s2(in, out, w_conv, coff, w_meta, moff);
}
