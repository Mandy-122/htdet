...
// Per-layer INT8 fake-quantization: clamp to [-127*s, +127*s] then round to scale grid.
// Used after each backbone layer to simulate W8A8 activation quantization.
// act_scale = max_observed_activation / 127  (from ptq_calibrate.py).
inline void fake_quant_buf(float* buf, int n, float act_scale) {
    float clamp = 127.0f * act_scale;
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < n; i++) {
        float v = buf[i];
        if (v >  clamp) v =  clamp;
        if (v < -clamp) v = -clamp;
        buf[i] = roundf(v / act_scale) * act_scale;
    }
}
