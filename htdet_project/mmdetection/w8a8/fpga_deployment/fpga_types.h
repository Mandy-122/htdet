...
//
// ============================================================
// W8A8 ACTIVE TYPES  (this deployment folder targets W8A8 synthesis)
// ============================================================

// INT8 conv weight type (BN-fused, per-channel symmetric)
typedef ap_int<8>   wint8_t;

// Per-layer activation types derived from PTQ calibration.
// Use the named alias matching each layer's INT_BITS from the table above.
typedef ap_fixed<8, 2, AP_RND, AP_SAT>  act8b2_t;  // retina_reg output
typedef ap_fixed<8, 3, AP_RND, AP_SAT>  act8b3_t;  // conv_proj, small lateral convs
typedef ap_fixed<8, 4, AP_RND, AP_SAT>  act8b4_t;  // later-stage expand, FPN outputs
typedef ap_fixed<8, 5, AP_RND, AP_SAT>  act8b5_t;  // most backbone 1×1 convs (majority)
typedef ap_fixed<8, 6, AP_RND, AP_SAT>  act8b6_t;  // stages 2–4 fusion, retina_cls
typedef ap_fixed<8, 7, AP_RND, AP_SAT>  act8b7_t;  // conv3_1x1 stage0, DW stage1.1
typedef float                            act8b9_t;  // DW outliers stages_0/1 — kept FP32

// W8A8 C-sim: use float for activations + explicit fake_quant_buf() per layer.
// ap_fixed<8,6> max=31.75 saturates stages_0.0 (ActMax=232) and conv3_1x1 (ActMax=50).
typedef float  act_t;

// Accumulator: wide enough to prevent overflow during INT8 MAC accumulation.
// INT8×INT8 = 16-bit product; accumulating 640 channels × 9 kernel = log2(5760)≈13 bits extra
// → total 8+8+13 = 29 bits → ap_int<32> is safe.
typedef ap_fixed<32, 16, AP_RND, AP_SAT>  acc_t;

// Input image: float32 pixels (ImageNet-normalised, range ≈ −2.1 to +2.6)
typedef float  input_t;

// Transformer layers: kept FP32 (attention Q·Kᵀ / softmax is range-sensitive)
typedef float  weight_t;   // Linear layer weights in transformer blocks
typedef float  bias_t;

// Bounding box and score types
// C-sim: float to avoid ap_fixed<16,10> saturation at 511.984 (2^9 - 2^-6).
// ap_fixed<16,10> max = 511.984 < 640 → all boxes with x/y > 512 saturate to x1=x2=511.984.
// For synthesis: use ap_fixed<24,12> (range ±2048, covers 640 with headroom).
typedef float  bbox_t;   // C-sim: float; synthesis: ap_fixed<24,12,AP_RND,AP_SAT>
// score_t: float during C-sim (ap_fixed<8,2> only has 6 frac bits = 0.015625 resolution).
typedef float  score_t;  // C-sim: float; synthesis: ap_fixed<16,4,AP_RND,AP_SAT>
