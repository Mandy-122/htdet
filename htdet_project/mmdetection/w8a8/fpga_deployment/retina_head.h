...
// Separated from retina_stacked_convs so src/dst are always concrete arrays —
// HLS does not support pointer selection (ternary on two different arrays).
static void retina_conv_once(
    ...
    for (int oc = 0; oc < HEAD_FEAT_CH; oc++) {
        CO_OH: for (int oh = 0; oh < H; oh++) {
            CO_OW: for (int ow = 0; ow < W; ow++) {
                #pragma HLS PIPELINE II=1
                acc_t acc = 0;
                CO_IC: for (int ic = 0; ic < HEAD_FEAT_CH; ic++) {
...
int process_level(
    ...
    // cls_feat: keep FP32 — quantizing before pred_conv collapses all scores to same bucket
    retina_stacked_convs(feat, reg_feat, reg_conv_w, H, W);
    // reg_feat: keep FP32 — quantizing amplifies exp() errors in box decoding
    ...
    // cls_logits: keep FP32 — quantizing with coarse scale flattens score distribution
    retina_pred_conv(reg_feat, reg_deltas, reg_pred_w, reg_pred_b, reg_out_ch, H, W);
    // reg_deltas: keep FP32 — exp(dw) amplifies any INT8 quantization error into huge bbox errors
    ...
    // ---- True topK anchor collection: scan ALL spatial positions, keep top-max_cand by score ----
    // No early exit on num_cand — raster truncation caused spatially-biased top-K.
    // When buffer fills: replace minimum entry if new score is higher (min-tracking replacement).
    // O(N) typical; O(N * max_cand) worst case (only when buffer overflows, rare at SCORE_THR=0.20).
    // NOTE: for synthesis, replace with a hardware-friendly priority queue or two-pass approach.
    int num_cand = 0;
    score_t buf_min     = (score_t)2.0f;  // tracks current minimum in buffer (> any sigmoid output)
    int     buf_min_idx = 0;

    SCORE_FILT: for (int h = 0; h < H; h++) {
        for (int w_i = 0; w_i < W; w_i++) {
            float cx = (w_i + 0.5f) * stride;
            float cy = (h   + 0.5f) * stride;

            for (int ri = 0; ri < ANCHOR_RATIOS_N; ri++) {
                for (int si = 0; si < ANCHOR_SCALES_N; si++) {
                    ...
                    for (int c = 0; c < NUM_CLASSES; c++) {
                        ...
                        score_t s = sigmoid((float)cls_logits[cls_ch * H * W + h * W + w_i]);
                        ...
                        // Tighter clamp: prevents exp() blowup while preserving useful range
                        if (dw < -2.5f) dw = -2.5f; if (dw > 2.5f) dw = 2.5f;
                        if (dh < -2.5f) dh = -2.5f; if (dh > 2.5f) dh = 2.5f;
                        ...
                        if (num_cand < max_cand) {
                            // Buffer not full: add directly, track running minimum
                            ...
                            // Buffer full: replace lowest-scoring entry with this higher-scoring one
                            cand_boxes[buf_min_idx*4+0] = (bbox_t)x1;
                            cand_boxes[buf_min_idx*4+1] = (bbox_t)y1;
                            cand_boxes[buf_min_idx*4+2] = (bbox_t)x2;
                            cand_boxes[buf_min_idx*4+3] = (bbox_t)y2;
                            cand_scores[buf_min_idx]    = s;
                            cand_cls   [buf_min_idx]    = c;
