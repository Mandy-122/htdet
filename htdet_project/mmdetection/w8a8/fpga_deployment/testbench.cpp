...
// Template: converts each element to float before writing, so this works for
// both float and ap_fixed<> activation types without changing call sites.
template<typename T>
void save_binary(const char* path, const T* data, int n_elems) {
    ...
    std::cout << "  saved " << n_elems << " floats → " << path
              << "  first5=[" << (float)data[0] << "," << (float)data[1] << ","
