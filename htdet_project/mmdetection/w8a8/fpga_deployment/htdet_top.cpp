...
void htdet_inference(
    // Input image: 3 × INPUT_H × INPUT_W, CHW, float32 (input_t=float for C-sim)
    const input_t   image      [INPUT_C * INPUT_H * INPUT_W],

    // Backbone weights (all stages, flat float32 stream; weight_t=float for C-sim)
    const weight_t  backbone_w [],   // 4,937,632 floats × 4B ≈ 19.8MB

    // FPN neck weights
    const weight_t  fpn_w      [],   // 2,598,912 floats × 4B ≈ 10.4MB

    // Head stacked conv weights (cls branch, 4 layers)
    const weight_t  cls_conv_w [],   // 2,361,344 floats × 4B ≈ 9.5MB

    // Head stacked conv weights (reg branch, 4 layers)
    const weight_t  reg_conv_w [],   // same ≈ 9.5MB
	...
	#pragma HLS INTERFACE m_axi port=fpn_w       offset=slave bundle=gmem2  depth=2598912
    ...
    // Sizes at float (4B): c1=6.4MB, c2=1.9MB, c3=410KB, c4=1.0MB
    ...
    // Sizes at float (4B): p2=26.2MB (synthesis-infeasible on ZU9EG — needs tiling),
    //   p3=6.6MB, p4=1.6MB, p5=410KB, p6=102KB
    // TODO(synthesis): tile P2/P3 or stream; full-frame float buffer won't close resources.
...
void htdet_backbone_only(
    ...
    #pragma HLS INTERFACE m_axi port=image      offset=slave bundle=gmem0  depth=IMAGE_ELEMS
    #pragma HLS INTERFACE m_axi port=backbone_w offset=slave bundle=gmem1  depth=4937632
    #pragma HLS INTERFACE m_axi port=c1         offset=slave bundle=gmem2  depth=C1_CH*C1_H*C1_W
    #pragma HLS INTERFACE m_axi port=c2         offset=slave bundle=gmem3  depth=C2_CH*C2_H*C2_W
    #pragma HLS INTERFACE m_axi port=c3         offset=slave bundle=gmem4  depth=C3_CH*C3_H*C3_W
    #pragma HLS INTERFACE m_axi port=c4         offset=slave bundle=gmem5  depth=C4_CH*C4_H*C4_W
    #pragma HLS INTERFACE s_axilite port=return bundle=control
    mobilevit_backbone(image, backbone_w, c1, c2, c3, c4);
}
...
void htdet_fpn_only(
    ...
    #pragma HLS INTERFACE m_axi port=c1    offset=slave bundle=gmem0  depth=C1_CH*C1_H*C1_W
    #pragma HLS INTERFACE m_axi port=c2    offset=slave bundle=gmem1  depth=C2_CH*C2_H*C2_W
    #pragma HLS INTERFACE m_axi port=c3    offset=slave bundle=gmem2  depth=C3_CH*C3_H*C3_W
    #pragma HLS INTERFACE m_axi port=c4    offset=slave bundle=gmem3  depth=C4_CH*C4_H*C4_W
    #pragma HLS INTERFACE m_axi port=fpn_w offset=slave bundle=gmem4  depth=2598912
    #pragma HLS INTERFACE m_axi port=p2    offset=slave bundle=gmem5  depth=P2_ELEMS
    #pragma HLS INTERFACE m_axi port=p3    offset=slave bundle=gmem6  depth=P3_ELEMS
    #pragma HLS INTERFACE m_axi port=p4    offset=slave bundle=gmem7  depth=P4_ELEMS
    #pragma HLS INTERFACE m_axi port=p5    offset=slave bundle=gmem8  depth=P5_ELEMS
    #pragma HLS INTERFACE m_axi port=p6    offset=slave bundle=gmem9  depth=P6_ELEMS
    #pragma HLS INTERFACE s_axilite port=return bundle=control
    fpn_neck(c1, c2, c3, c4, fpn_w, p2, p3, p4, p5, p6);
}
