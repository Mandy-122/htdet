/*
 * fpga_utils.h  (PTQ INT8 — v5, OC_TILE=8, MAX_IC_TILES=32)
 *
 * mbconv_160 Run 2: OC_TILE increased 4→8.
 * Halves expand outer iterations (32→16 OC groups × 25,600 HW).
 * Expected IC_T II=4 (8 weight accesses / 2 BRAM ports) but outer trip
 * savings (2×) dominate → ~2× speedup on expand vs Run 1 (OC_TILE=4).
 *
 * Layout: HWC [H*W * ch] for all 1x1 and DW conv activations.
 * Tree: 5-level binary tree (32→16→8→4→2→1) for partial sum reduction,
 *       now applied to 8 OC streams in parallel.
 */
...
// ============================================================
// 1x1 CONV + BN + SiLU  (HWC, OC_TILE=8, partial acc, 5-level tree)
// input/output: HWC [H*W * ch]
// Caller partitions: in cyclic factor=16, w_conv cyclic factor=16
// 8 OC streams: IC_T II=4 (8 weight accesses / 2 BRAM ports per bank),
// but outer trip halved vs OC_TILE=4 → net ~2× speedup on expand.
// ============================================================
inline void conv1x1_bn_silu(
    ...
    C1X1S_OC: for (int oc = 0; oc < out_ch; oc += OC_TILE) {
        C1X1S_HW: for (int hw = 0; hw < H * W; hw++) {
            ...
            acc_t partial4[MAX_IC_TILES], partial5[MAX_IC_TILES];
            acc_t partial6[MAX_IC_TILES], partial7[MAX_IC_TILES];
            ...
            #pragma HLS ARRAY_PARTITION variable=partial4 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=partial5 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=partial6 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=partial7 complete dim=1

            acc_t l0_0[16], l0_1[16], l0_2[16], l0_3[16];
            acc_t l0_4[16], l0_5[16], l0_6[16], l0_7[16];
            acc_t l1_0[ 8], l1_1[ 8], l1_2[ 8], l1_3[ 8];
            acc_t l1_4[ 8], l1_5[ 8], l1_6[ 8], l1_7[ 8];
            acc_t l2_0[ 4], l2_1[ 4], l2_2[ 4], l2_3[ 4];
            acc_t l2_4[ 4], l2_5[ 4], l2_6[ 4], l2_7[ 4];
            acc_t l3_0[ 2], l3_1[ 2], l3_2[ 2], l3_3[ 2];
            acc_t l3_4[ 2], l3_5[ 2], l3_6[ 2], l3_7[ 2];
            ...
            #pragma HLS ARRAY_PARTITION variable=l0_4 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=l0_5 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=l0_6 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=l0_7 complete dim=1
            ...
            #pragma HLS ARRAY_PARTITION variable=l1_4 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=l1_5 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=l1_6 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=l1_7 complete dim=1
            ...
            #pragma HLS ARRAY_PARTITION variable=l2_4 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=l2_5 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=l2_6 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=l2_7 complete dim=1
            ...
            #pragma HLS ARRAY_PARTITION variable=l3_4 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=l3_5 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=l3_6 complete dim=1
            #pragma HLS ARRAY_PARTITION variable=l3_7 complete dim=1

            C1X1S_INIT: for (int t = 0; t < MAX_IC_TILES; t++) {
                #pragma HLS UNROLL
                partial0[t] = 0.0f; partial1[t] = 0.0f;
                partial2[t] = 0.0f; partial3[t] = 0.0f;
                partial4[t] = 0.0f; partial5[t] = 0.0f;
                partial6[t] = 0.0f; partial7[t] = 0.0f;
            }

            C1X1S_IC_T: for (int t = 0; t < in_ch / IC_TILE; t++) {
                #pragma HLS PIPELINE II=1
                acc_t tile0=0.0f, tile1=0.0f, tile2=0.0f, tile3=0.0f;
                acc_t tile4=0.0f, tile5=0.0f, tile6=0.0f, tile7=0.0f;
                C1X1S_IC_K: for (int k = 0; k < IC_TILE; k++) {
                    ...
                    tile4 += x * (acc_t)(float)weights[(oc + 4) * in_ch + t * IC_TILE + k];
                    tile5 += x * (acc_t)(float)weights[(oc + 5) * in_ch + t * IC_TILE + k];
                    tile6 += x * (acc_t)(float)weights[(oc + 6) * in_ch + t * IC_TILE + k];
                    tile7 += x * (acc_t)(float)weights[(oc + 7) * in_ch + t * IC_TILE + k];
                }
                partial0[t] = tile0; partial1[t] = tile1;
                partial2[t] = tile2; partial3[t] = tile3;
                partial4[t] = tile4; partial5[t] = tile5;
                partial6[t] = tile6; partial7[t] = tile7;
            }

            // 5-level binary tree reduction (32→16→8→4→2→1), 8 OC streams in parallel
            for (int i = 0; i < 16; i++) {
                #pragma HLS UNROLL
                ...
                l0_4[i] = partial4[2*i] + partial4[2*i+1];
                l0_5[i] = partial5[2*i] + partial5[2*i+1];
                l0_6[i] = partial6[2*i] + partial6[2*i+1];
                l0_7[i] = partial7[2*i] + partial7[2*i+1];
            }
            for (int i = 0; i < 8; i++) {
                #pragma HLS UNROLL
                ...
                l1_4[i] = l0_4[2*i] + l0_4[2*i+1];
                l1_5[i] = l0_5[2*i] + l0_5[2*i+1];
                l1_6[i] = l0_6[2*i] + l0_6[2*i+1];
                l1_7[i] = l0_7[2*i] + l0_7[2*i+1];
            }
            for (int i = 0; i < 4; i++) {
                #pragma HLS UNROLL
                ...
                l2_4[i] = l1_4[2*i] + l1_4[2*i+1];
                l2_5[i] = l1_5[2*i] + l1_5[2*i+1];
                l2_6[i] = l1_6[2*i] + l1_6[2*i+1];
                l2_7[i] = l1_7[2*i] + l1_7[2*i+1];
            }
            for (int i = 0; i < 2; i++) {
                #pragma HLS UNROLL
                ...
                l3_4[i] = l2_4[2*i] + l2_4[2*i+1];
                l3_5[i] = l2_5[2*i] + l2_5[2*i+1];
                l3_6[i] = l2_6[2*i] + l2_6[2*i+1];
                l3_7[i] = l2_7[2*i] + l2_7[2*i+1];
            }
            acc_t acc0 = l3_0[0]+l3_0[1]; acc_t acc1 = l3_1[0]+l3_1[1];
            acc_t acc2 = l3_2[0]+l3_2[1]; acc_t acc3 = l3_3[0]+l3_3[1];
            acc_t acc4 = l3_4[0]+l3_4[1]; acc_t acc5 = l3_5[0]+l3_5[1];
            acc_t acc6 = l3_6[0]+l3_6[1]; acc_t acc7 = l3_7[0]+l3_7[1];
            ...
            output[hw*out_ch + oc + 4] = silu((act_t)((acc_t)acc4*(acc_t)bn_scale[oc + 4]+(acc_t)bn_bias[oc + 4]));
            output[hw*out_ch + oc + 5] = silu((act_t)((acc_t)acc5*(acc_t)bn_scale[oc + 5]+(acc_t)bn_bias[oc + 5]));
            output[hw*out_ch + oc + 6] = silu((act_t)((acc_t)acc6*(acc_t)bn_scale[oc + 6]+(acc_t)bn_bias[oc + 6]));
            output[hw*out_ch + oc + 7] = silu((act_t)((acc_t)acc7*(acc_t)bn_scale[oc + 7]+(acc_t)bn_bias[oc + 7]));
        }
    }
}

// ============================================================
// 1x1 CONV + BN  (HWC, OC_TILE=8, partial acc, 5-level tree, no activation)
// ============================================================
inline void conv1x1_bn(
    ...
    C1X1_OC: for (int oc = 0; oc < out_ch; oc += OC_TILE) {
        C1X1_HW: for (int hw = 0; hw < H * W; hw++) {
            ...
            output[hw*out_ch + oc + 4] = (act_t)((acc_t)acc4*(acc_t)bn_scale[oc + 4]+(acc_t)bn_bias[oc + 4]);
            output[hw*out_ch + oc + 5] = (act_t)((acc_t)acc5*(acc_t)bn_scale[oc + 5]+(acc_t)bn_bias[oc + 5]);
            output[hw*out_ch + oc + 6] = (act_t)((acc_t)acc6*(acc_t)bn_scale[oc + 6]+(acc_t)bn_bias[oc + 6]);
            output[hw*out_ch + oc + 7] = (act_t)((acc_t)acc7*(acc_t)bn_scale[oc + 7]+(acc_t)bn_bias[oc + 7]);
        }
    }
}
