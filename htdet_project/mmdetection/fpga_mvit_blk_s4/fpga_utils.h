/*
 * fpga_utils.h  (PTQ INT8 variant)
 * Activation functions, normalization, and convolution primitives.
 * All functions are HLS-inlined; used by backbone, FPN, and head.
 *
 * Changes vs float32 fpga_192/fpga_utils.h:
 *   - conv primitive signatures: bn_scale/bn_bias are const meta_t* (float)
 *   - MAC inner loop: explicit (float)weights[...] cast before multiply
 *   - fused_bn: scale is meta_t (float)
 *   - linear_layer/layer_norm_seq: weights/bias are const meta_t*
 *   - ARRAY_PARTITION: cyclic factor=8 → factor=16 (matches UNROLL factor=16)
 *   - #pragma HLS RESOURCE → #pragma HLS bind_storage
 *   - silu/sigmoid: #ifdef __SYNTHESIS__ guard for piecewise-linear approx
 */
...
// Synthesis uses piecewise-linear approximation; C-sim uses exact.
...
//   eff_scale[oc] = w_dequant_scale[oc] * gamma[oc] / sqrt(var[oc] + eps)
//   eff_bias[oc]  = beta[oc] - mean[oc] * gamma[oc] / sqrt(var[oc] + eps)
// Both scale and bias are pre-computed in export_ptq_meta.py.
// scale is meta_t (float) for W8A32.
// ============================================================
...
// weight/bias:  [dim]  — meta_t (float) for W8A32
// ============================================================
inline void layer_norm_seq(
    const act_t* input,
    act_t*       output,
    const meta_t* weight,
    const meta_t* bias_ln,
    int seq_len, int dim
) {
    #pragma HLS INLINE
...
// weights: const weight_t* (int8_t)
// bn_scale, bn_bias: const meta_t* (float — pre-computed effective BN params)
// MAC: acc += (acc_t)input[...] * (acc_t)(float)weights[...]
//      (explicit int8_t → float cast before multiply)
// ============================================================
                        ...
                        C3X3_KW: for (int kw = 0; kw < 3; kw++) {
                            ...
                            if (ih >= 0 && ih < H && iw >= 0 && iw < W) {
                                acc += (acc_t)input[ic * H * W + ih * W + iw]
                                     * (acc_t)(float)weights[(oc * in_ch + ic) * 9 + kh * 3 + kw];
                            }
                        }
                    }
                }
...
// 3x3 conv + fused BN (no activation)
        ...
        C1X1S_HW: for (int hw = 0; hw < H * W; hw++) {
            ...
            C1X1S_IC: for (int ic = 0; ic < in_ch; ic++) {
                ...
                acc += (acc_t)input[ic * H * W + hw]
                     * (acc_t)(float)weights[oc * in_ch + ic];
            }
            act_t y = (act_t)((acc_t)acc * (acc_t)bn_scale[oc] + (acc_t)bn_bias[oc]);
...
// 1x1 conv + fused BN (no activation)
...
// 1x1 conv, no BN, no activation (projection layers, FPN lateral, RetinaHead pred)
// weights: int8, bias: meta_t (float)
...
// Depthwise 3x3 conv + fused BN (no activation)
...
// weights: [out_dim * in_dim] meta_t (float — transformer weights are float)
// bias: [out_dim] meta_t (float)
inline void linear_layer(
    const act_t*   input,
    act_t*         output,
    const meta_t*  weights,
    const meta_t*  bias,
