/*
 * mvit_blk_s3_top.cpp
 * Full MobileViT Block Stage-3. Self-contained.
 * in_ch=128, d=192, H=W=20, depth=4, p=2, N=100 tokens per view
 */

#include "mvit_blk_s3_top.h"
#include "fpga_utils.h"
#include <cmath>

static void transformer_block_s3(act_t* tokens, const meta_t* w) {
    #pragma HLS INLINE
    ...
    const int N = MVIT_N_S3, dim = MVIT_S3_DIM, head_dim = dim / MVIT_HEADS;
    int off = 0;
    ...
    S3T_MHA: for (int h = 0; h < MVIT_HEADS; h++) {
        ...
        S3T_AQ: for (int q = 0; q < N; q++) {
            S3T_AK: for (int k = 0; k < N; k++) {
                ...
                acc_t dot = 0;
                S3T_AD: for (int d = 0; d < head_dim; d++) { #pragma HLS UNROLL factor=4
                    dot += (acc_t)s3t_q_buf[q*dim+hoff+d] * (acc_t)s3t_k_buf[k*dim+hoff+d]; }
                s3t_attn_scores[q*N+k] = (act_t)((float)dot * scale);
            }
        }
        S3T_SM: for (int q = 0; q < N; q++) softmax_row(s3t_attn_scores+q*N, s3t_attn_scores+q*N, N);
        S3T_VQ: for (int q = 0; q < N; q++) {
            S3T_VD: for (int d = 0; d < head_dim; d++) {
                #pragma HLS PIPELINE II=1
                acc_t sum = 0;
                S3T_VK: for (int k = 0; k < N; k++) { #pragma HLS UNROLL factor=4
    ...
    S3T_R1: for (int i = 0; i < N*dim; i++) { #pragma HLS PIPELINE II=1 tokens[i] += s3t_proj_out[i]; }
    ...
    S3T_M1B: for (int b = 0; b < N; b++) {
        S3T_M1O: for (int od = 0; od < hidden; od++) {
            ...
            S3T_M1I: for (int id = 0; id < dim; id++) { #pragma HLS UNROLL factor=8
    ...
    S3T_M2B: for (int b = 0; b < N; b++) {
        S3T_M2O: for (int od = 0; od < dim; od++) {
            ...
            S3T_M2I: for (int id = 0; id < hidden; id++) { #pragma HLS UNROLL factor=8
                acc += (acc_t)s3t_mlp_hid[b*hidden+id] * (acc_t)W2[od*hidden+id]; }
            s3t_proj_out[b*dim+od] = (act_t)acc;
        }
    }
    S3T_R2: for (int i = 0; i < N*dim; i++) { #pragma HLS PIPELINE II=1 tokens[i] += s3t_proj_out[i]; }
}

static void mobilevit_block_s3(
...
void mvit_blk_s3_top(
    ...
    int coff = 0, moff = 0;
    mobilevit_block_s3(in, out, w_conv, coff, w_meta, moff);
}
